document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateModeDisplay() {
        if (modeDisplayBtn) {
            let modeText = '';
            if (appSettings.actionMode === 'md') modeText = 'Add .md';
            else if (appSettings.actionMode === 'txt') modeText = 'Add .txt';
            else if (appSettings.actionMode === 'detect') modeText = 'Detect Info';
            ...
            await Promise.all(batch.map(async (itemDiv) => {
                ...
                if (action === 'detect') {
                            ...
                            if (isUtf8) {
                                ...
                                for (let j = 0; j < view.length; j++) {
                                    ...
                                    if ((b >= 0x81 && b <= 0x9F) || (b >= 0xE0 && b <= 0xFC)) {
                                        ...
                                        // Hankaku Kana usually 0xA1-0xDF, but strictly non-ASCII
                                        if (b < 0xA1 || b > 0xDF) {
                                            // 0x80 or 0xA0 are invalid in plain SJIS text usually
                                            // validSJIS = false; 
                                            // But allow for now
                                        }
                                    } else if (b >= 0xFD) {
                    ...
                    if (nameSpan) {
                        ...
                        nameSpan.after(badgeEnc); // Insert Enc before EOL conceptually, but after name
                    }

                } else if (isText && !lowercaseName.endsWith(targetExtension)) {
                    // Start of original rename logic...
